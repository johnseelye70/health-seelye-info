...
import { SimpleMovementActivity } from '@/lib/movement-database';

interface PreMadeProgramsBrowserProps {
  onBackToMovementChoices?: () => void;
}

export const PreMadeProgramsBrowser: React.FC<PreMadeProgramsBrowserProps> = ({
  onBackToMovementChoices,
}) => {
  const { profile, activeProgramId, setActiveProgramId, addSimpleMovementActivity } = useHealth();
  ...
  const handleAddDayToMovement = (prog: PreMadeWorkoutProgram, day: WorkoutProgramDay) => {
    const isCardio =
    ...
    const activity: SimpleMovementActivity = {
      ...
      description: `${day.focus}. Features: ${day.exercises.slice(0, 4).map((e) => e.name).join(', ')}${day.exercises.length > 4 ? ` (+${day.exercises.length - 4} more)` : ''}.`,
    ...
    setActionToast(`Added "${activity.title}" to Today's Movement Choices!`);
      ...
      <div className="p-6 md:p-8 rounded-3xl bg-surface-100/90 border border-surface-border backdrop-blur-xl space-y-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          {onBackToMovementChoices && (
            <button
              type="button"
              id="back-to-movement-choices-btn"
              onClick={onBackToMovementChoices}
              className="flex items-center gap-1.5 px-4 py-2.5 rounded-2xl bg-surface-200 hover:bg-surface-300 border border-surface-border text-xs font-bold text-zinc-200 hover:text-white transition-all active:scale-95 cursor-pointer shrink-0 self-start md:self-auto"
            >
              <ArrowLeft className="w-4 h-4 text-accent-coral" />
              <span>Back to Daily Movement</span>
            </button>
          )}
        </div>
        ...
        <div className="p-4 rounded-2xl bg-emerald-500/20 border border-emerald-500/40 text-emerald-300 text-xs font-bold flex items-center justify-between gap-3 animate-fadeIn shadow-lg">
          ...
          {onBackToMovementChoices && (
            <button
              type="button"
              onClick={onBackToMovementChoices}
              className="px-3 py-1.5 rounded-xl bg-emerald-500 text-zinc-950 text-xs font-black hover:bg-emerald-400 transition-all cursor-pointer shadow-glow"
            >
              View in Today's Movement →
            </button>
          )}
        </div>
      )}
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      if (prog.schedule[0]) {
                        handleAddDayToMovement(prog, prog.schedule[0]);
                      }
                    }}
                    className="px-2.5 py-1 rounded-xl bg-accent-coral/15 hover:bg-accent-coral/25 border border-accent-coral/30 text-accent-coral text-[11px] font-bold transition-all flex items-center gap-1 cursor-pointer active:scale-95"
                    title="Add Day 1 to Today's Movement Choices"
                  >
                ...
                {activeProgramModal.schedule.map((day) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 self-start sm:self-auto shrink-0">
                      <button
                        type="button"
                        onClick={() => handleAddDayToMovement(activeProgramModal, day)}
                        className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-accent-coral/20 hover:bg-accent-coral/30 border border-accent-coral/40 text-accent-coral text-xs font-bold transition-all cursor-pointer active:scale-95"
                        title="Add this session to Today's Movement Choices"
                      >
