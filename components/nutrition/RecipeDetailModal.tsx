...
export const RecipeDetailModal: React.FC<RecipeDetailModalProps> = ({
  ...
  return (
    <>
      ...
      >
        <div
          className="relative w-full max-w-3xl max-h-[92vh] flex flex-col rounded-3xl bg-surface-100 border border-surface-border shadow-2xl text-foreground overflow-hidden"
        ...
        >
          ...
          <div className="p-5 sm:p-6 border-b border-surface-border shrink-0 flex items-start justify-between gap-4 bg-surface-100">
            ...
            <button
              onClick={onClose}
              className="p-2.5 rounded-2xl text-zinc-400 hover:text-foreground bg-surface-200 hover:bg-surface-300 transition-colors shrink-0 cursor-pointer shadow-sm active:scale-95"
              title="Close recipe"
          ...
          <div className="p-5 sm:p-6 overflow-y-auto flex-1 space-y-6">
            ...
            <div className="flex flex-wrap items-center gap-2.5 text-xs font-mono text-zinc-400">
              <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-surface-200 border border-surface-border">
                <Clock className="w-3.5 h-3.5 text-zinc-400" />
                <span>Prep: {recipe.prep_time_minutes}m • Cook: {recipe.cook_time_minutes}m</span>
              </div>

              <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-surface-200 border border-surface-border text-brand-400 font-bold">
            ...
            <div className="p-4 rounded-2xl bg-surface-200/70 border border-surface-border flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="space-y-1">
                ...
                <div className="text-xs text-zinc-400">
                  Yield: <strong className="text-foreground">{recipe.servings_yield * batchMultiplier} portion{recipe.servings_yield * batchMultiplier > 1 ? 's' : ''}</strong> ({totalCalories} total kcal)
                </div>
              </div>
                    ...
                    <div
                      ...
                      className={`p-3 rounded-2xl border transition-all cursor-pointer flex items-center justify-between gap-3 ${
                        isChecked
                          ...
                          : 'bg-surface-200/60 border-surface-border text-foreground hover:border-zinc-700'
                      }`}
                    >
          ...
          <div className="p-4 sm:p-5 border-t border-surface-border shrink-0 bg-surface-100 flex flex-col sm:flex-row items-center justify-between gap-3">
            <div className="flex items-center gap-2 w-full sm:w-auto">
              ...
              <button
                type="button"
                onClick={onClose}
                className="w-full sm:w-auto px-4 py-2.5 rounded-2xl bg-surface-200 hover:bg-surface-300 border border-surface-border text-xs font-bold text-zinc-300 hover:text-foreground flex items-center justify-center gap-2 transition-all active:scale-95 cursor-pointer shadow-sm"
              >
                <X className="w-4 h-4" />
                <span>Close</span>
              </button>

              {/* Print Button (Direct Printer Trigger) */}
              <button
                type="button"
                onClick={handleDirectPrint}
                className="w-full sm:w-auto px-4 py-2.5 rounded-2xl bg-surface-200 hover:bg-surface-300 border border-surface-border text-xs font-bold text-foreground flex items-center justify-center gap-2 transition-all active:scale-95 cursor-pointer shadow-sm"
              >
            ...
            <div className="flex items-center gap-2 w-full sm:w-auto">
              ...
              <button
                ...
                className="flex-1 sm:flex-none px-4 py-2.5 rounded-2xl bg-surface-200 hover:bg-surface-300 border border-surface-border text-xs font-bold text-foreground flex items-center justify-center gap-2 transition-all active:scale-95 cursor-pointer"
              >
          ...
          <div className="border-b-2 border-black pb-2 mb-2 flex items-start justify-between">
            ...
            <div className="text-right shrink-0">
              <div className="text-xs font-black text-black">
                {recipe.calories_per_serving * batchMultiplier} kcal
              </div>
              <div className="text-[9px] text-zinc-700 font-mono">
                {recipe.prep_time_minutes + recipe.cook_time_minutes} min • {recipe.servings_yield * batchMultiplier} serving{recipe.servings_yield * batchMultiplier > 1 ? 's' : ''}
              </div>
            </div>
          </div>
