...
export const RecipePrintModal: React.FC<RecipePrintModalProps> = ({
  ...
  return (
    <>
      {/* Interactive Modal UI (Hidden during window.print) */}
      <div className="no-print fixed inset-0 z-50 bg-black/85 backdrop-blur-md flex items-center justify-center p-3 sm:p-6 overflow-y-auto animate-fadeIn select-none">
        <div
          className="relative w-full max-w-4xl my-auto p-6 sm:p-8 rounded-3xl bg-surface-100/95 border border-surface-border shadow-2xl backdrop-blur-xl text-zinc-100 max-h-[90vh] overflow-y-auto space-y-6"
        ...
        >
          ...
          <div className="flex items-center justify-between pb-4 border-b border-surface-border">
            ...
            <button
              onClick={onClose}
              className="p-2.5 rounded-2xl text-zinc-400 hover:text-white bg-surface-200/80 hover:bg-surface-200 transition-colors cursor-pointer"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between text-xs font-bold text-zinc-300">
              ...
              <span className="text-zinc-500 text-[11px] font-normal">
                {printFormat === 'index_card_4x6'
                  ? 'Formatted for standard 4" x 6" recipe card boxes'
                  : 'Formatted for full 8.5" x 11" binder sheet'}
              </span>
            </div>

            <div className="p-4 sm:p-6 rounded-2xl bg-zinc-900 border border-zinc-800 flex items-center justify-center overflow-x-auto">
              {/* The Visual Paper Container */}
              <div
                className={`bg-white text-zinc-950 shadow-2xl transition-all duration-300 ${
                  printFormat === 'index_card_4x6'
                    ? 'w-full max-w-[580px] p-5 border-2 border-zinc-950 rounded-xl'
                    : 'w-full max-w-[700px] p-8 border-2 border-zinc-300 rounded-xl'
                }`}
                style={{
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-2">
            ...
            <button
              type="button"
              onClick={handlePrint}
              className="w-full sm:w-auto px-6 py-3 rounded-2xl bg-gradient-to-r from-brand-500 to-accent-teal hover:from-brand-600 hover:to-accent-teal/90 text-zinc-950 font-bold text-xs shadow-glow transition-all active:scale-95 flex items-center justify-center gap-2 cursor-pointer"
            >
